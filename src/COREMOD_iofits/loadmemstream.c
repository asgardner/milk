...
imageID COREMOD_IOFITS_LoadMemStream(const char *sname, uint64_t *streamflag,
                                     uint32_t *imLOC) {
  imageID ID = 0;
  ...
  *imLOC = STREAM_LOAD_SOURCE_NOTFOUND;
      ...
      if (imLOCALMEM == 0) {
        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
        if (MEMLOADREPORT) {
          char msg[STRINGMAXLEN_FPS_LOGMSG];
          SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                         "%s EXITFAIL STREAM_LOAD_FORCE_LOCALMEM: Image does "
                         "not exist in local memory",
                         sname);
          functionparameter_outlog("LOADMEMSTREAM", msg);
        }
      } else {
      ...
      if (ID == -1) {
        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
        if (MEMLOADREPORT == 1) {
          char msg[STRINGMAXLEN_FPS_LOGMSG];
          SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                         "%s EXITFAIL STREAM_LOAD_FORCE_SHAREDMEM: Image does "
                         "not exist in shared memory",
                         sname);
          functionparameter_outlog("LOADMEMSTREAM", msg);
        }
      } else {
        *imLOC = STREAM_LOAD_SOURCE_SHAREMEM;
        if (MEMLOADREPORT == 1) {
          ...
          functionparameter_outlog("LOADMEMSTREAM", msg);
          SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname,
                         *imLOC);
          functionparameter_outlog("LOADMEMSTREAM", msg);
        }
      }
      ...
      if (ID == -1) {
        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
        if (MEMLOADREPORT == 1) {
          char msg[STRINGMAXLEN_FPS_LOGMSG];
          SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                         "%s EXITFAIL STREAM_LOAD_FORCE_CONFFITS: Image does "
                         "not exist as conf FITS",
                         sname);
          functionparameter_outlog("LOADMEMSTREAM", msg);
        }
      } else {
      ...
      if (fp == NULL) {
        ...
        *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
        if (MEMLOADREPORT == 1) {
          char msg[STRINGMAXLEN_FPS_LOGMSG];
          SNPRINTF_CHECK(
              msg, STRINGMAXLEN_FPS_LOGMSG,
              "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: File %s does not exist",
              sname, fname);
          functionparameter_outlog("LOADMEMSTREAM", msg);
          SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname,
                         *imLOC);
          ...
          if (ferror(fp)) {
            ...
            *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
            if (MEMLOADREPORT == 1) {
              char msg[STRINGMAXLEN_FPS_LOGMSG];
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                             "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: fscanf "
                             "cannot read stream fname",
                             sname);
              functionparameter_outlog("LOADMEMSTREAM", msg);
            }
          } else {
            ...
            *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
            if (MEMLOADREPORT == 1) {
              ...
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                             "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: fscanf "
                             "reached end of file, no matching characters",
                             sname);
              functionparameter_outlog("LOADMEMSTREAM", msg);
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname,
                             *imLOC);
            ...
            if (ID == -1) {
              *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
              if (MEMLOADREPORT == 1) {
                char msg[STRINGMAXLEN_FPS_LOGMSG];
                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                               "%s EXITFAIL STREAM_LOAD_FORCE_CONFNAME: cannot "
                               "load stream fname",
                               sname);
                functionparameter_outlog("LOADMEMSTREAM", msg);
                SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u",
                               sname, *imLOC);
                functionparameter_outlog("LOADMEMSTREAM", msg);
              }
            } else {
  ...
  if (*imLOC == STREAM_LOAD_SOURCE_NOTFOUND)  // still searching
  {
    ...
    if (!(FPFLAG_STREAM_LOAD_SKIPSEARCH_LOCALMEM & *streamflag)) {
      if (MEMLOADREPORT == 1) {
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s Search LOCALMEM",
                       sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
      if (imLOCALMEM == 1) {
        *imLOC = STREAM_LOAD_SOURCE_LOCALMEM;
        ...
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                       "%s localmem stream not found", sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
    }
    // printf("imLOC = %d\n", *imLOC);
  ...
  if (*imLOC == STREAM_LOAD_SOURCE_NOTFOUND)  // still searching
  {
    ...
    if (!(FPFLAG_STREAM_LOAD_SKIPSEARCH_SHAREMEM & *streamflag)) {
      if (MEMLOADREPORT == 1) {
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s Search SHAREMEM",
                       sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
      ID = read_sharedmem_image(sname);
  ...
  if (*imLOC == STREAM_LOAD_SOURCE_NOTFOUND)  // still searching
  {
    if (!(FPFLAG_STREAM_LOAD_SKIPSEARCH_CONFFITS & *streamflag)) {
      if (MEMLOADREPORT == 1) {
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s Search CONFFITS",
                       sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
      // printf("imLOC = %d\n", *imLOC);
      ...
      if (ID != -1) {
        *imLOC = STREAM_LOAD_SOURCE_CONFFITS;
        ...
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s File %s not found",
                       sname, fname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
      // printf("imLOC = %d\n", *imLOC);
  ...
  if (*imLOC == STREAM_LOAD_SOURCE_NOTFOUND)  // still searching
  {
    if (!(FPFLAG_STREAM_LOAD_SKIPSEARCH_CONFNAME & *streamflag)) {
      if (MEMLOADREPORT == 1) {
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s Search CONFNAME",
                       sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
          ...
          if (ferror(fp)) {
            ...
            *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
            if (MEMLOADREPORT == 1) {
              char msg[STRINGMAXLEN_FPS_LOGMSG];
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                             "%s EXITFAILURE fscanf error reading %s", sname,
                             fname);
              functionparameter_outlog("LOADMEMSTREAM", msg);
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname,
                             *imLOC);
            ...
            *imLOC = STREAM_LOAD_SOURCE_EXITFAILURE;  // fail
            if (MEMLOADREPORT == 1) {
              char msg[STRINGMAXLEN_FPS_LOGMSG];
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG,
                             "%s EXITFAILURE fscanf error reading %s. fscanf "
                             "reached end of file, no matching characters",
                             sname, fname);
              functionparameter_outlog("LOADMEMSTREAM", msg);
              SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s imLOC %u", sname,
      ...
      if (*imLOC == STREAM_LOAD_SOURCE_NOTFOUND) {
        if (fscanfcnt > 0) {
          {
            char msg[STRINGMAXLEN_FPS_LOGMSG];
            SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s LOADING %s", sname,
                           streamfname);
            functionparameter_outlog("LOADMEMSTREAM", msg);
            load_fits(streamfname, sname, 0, &ID);
            if (ID != -1) {
  ...
  if (*imLOC == STREAM_LOAD_SOURCE_LOCALMEM)
    if (FPFLAG_STREAM_LOAD_UPDATE_SHAREMEM & *streamflag) {
      if (MEMLOADREPORT == 1) {
        char msg[STRINGMAXLEN_FPS_LOGMSG];
        SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s copy to SHAREMEM",
                       sname);
        functionparameter_outlog("LOADMEMSTREAM", msg);
      }
  ...
  if (updateCONFFITS == 1) {
    ...
    if (MEMLOADREPORT == 1) {
      char msg[STRINGMAXLEN_FPS_LOGMSG];
      SNPRINTF_CHECK(msg, STRINGMAXLEN_FPS_LOGMSG, "%s copy to CONFFITS",
                     sname);
      functionparameter_outlog("LOADMEMSTREAM", msg);
    }
    WRITE_FULLFILENAME(fname, "./conf/shmim.%s.fits", sname);
