...
errno_t processinfo_procdirname(char *procdname) {
  ...
  if (MILK_PROC_DIR != NULL) {
    ...
    {
      int slen =
          snprintf(procdname, STRINGMAXLEN_FULLFILENAME, "%s", MILK_PROC_DIR);
      if (slen < 1) {
...
PROCESSINFO *processinfo_setup(
    ...
    const char *descriptionstring, const char *msgstring,
  ...
  if (data.processinfoActive == 0) {
    ...
    {
      int slen =
          snprintf(pinfoname0, STRINGMAXLEN_PROCESSINFO_NAME, "%s", pinfoname);
      if (slen < 1) {
        PRINT_ERROR("snprintf wrote <1 char");
        abort();  // can't handle this error any other way
      }
...
PROCESSINFO *processinfo_shm_create(const char *pname, int CTRLval) {
  ...
  static int LogFileCreated = 0;  // toggles to 1 when created. To avoid
                                  // re-creating file on same process
  ...
  strncpy(pinfolist->pnamearray[pindex], pname,
  ...
  {
    int slen =
        snprintf(pinfo->logfilename, STRINGMAXLEN_PROCESSINFO_LOGFILENAME,
...
PROCESSINFO *processinfo_shm_link(const char *pname, int *fd) {
  ...
  *fd = open(pname, O_RDWR);
  ...
  PROCESSINFO *pinfolist = (PROCESSINFO *)mmap(
      0, file_stat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, *fd, 0);
  if (pinfolist == MAP_FAILED) {
    close(*fd);
  ...
  if (processinfo->loopstat != 4) {
    ...
    if (processinfo->CTRLval == 3)  // loop exit from processinfo control
    {
      sprintf(msgstring, "CTRLexit %02d:%02d:%02d.%03d", tstoptm->tm_hour,
              tstoptm->tm_min, tstoptm->tm_sec,
              (int)(0.000001 * (tstop.tv_nsec)));
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wstringop-truncation"
      strncpy(processinfo->statusmsg, msgstring,
...
int processinfo_WriteMessage(PROCESSINFO *processinfo, const char *msgstring) {
  ...
  fprintf(processinfo->logFile, "%02d:%02d:%02d.%06d  %8ld.%09ld  %06d  %s\n",
          ...
          (int)(0.001 * (tnow.tv_nsec)), tnow.tv_sec, tnow.tv_nsec,
  ...
  if (processinfo->MeasureTiming == 1) {
    ...
    if (processinfo->dtiter_limit_enable != 0) {
      ...
      if (dtiter > processinfo->dtiter_limit_value) {
        ...
        {
          int slen =
              snprintf(msgstring, STRINGMAXLEN_PROCESSINFO_STATUSMSG,
                       "dtiter %4ld  %4d %6.1f us  > %6.1f us",
                       processinfo->dtiter_limit_cnt, processinfo->timerindex,
...
int processinfo_exec_end(PROCESSINFO *processinfo) {
  ...
  if (processinfo->MeasureTiming == 1) {
    ...
    if (processinfo->dtexec_limit_enable != 0) {
      ...
      dtexec +=
          1000000000 * (processinfo->texecend[processinfo->timerindex].tv_sec -
...

        {
          int slen =
              snprintf(msgstring, STRINGMAXLEN_PROCESSINFO_STATUSMSG,
                       ...
                       processinfo->dtexec_limit_cnt, processinfo->timerindex,
...
(ret==12) )
           {
                           if(startScan == 1)
                           {
                                   ret = sscanf(outstring, "%d %s %s %d %s %s %s
%s %f %f %s %s\n", &toparray_PID[NBtop], toparray_USER[NBtop],
                                                toparray_PR[NBtop],
                                                &toparray_NI[NBtop],
                                                 toparray_VIRT[NBtop],
                                                 toparray_RES[NBtop],
...
static int GetCPUloads(PROCINFOPROC *pinfop) {
  ...
  while (((read = getline(&line, &maxstrlen, fp)) != -1) &&
         (cpu < pinfop->NBcpus)) {
    sscanf(line, "%s %lld %lld %lld %lld %lld %lld %lld %lld %lld", string0,
           &vall0, &vall1, &vall2, &vall3, &vall4, &vall5, &vall6, &vall7,
           &vall8);
  ...
  EXECUTE_SYSTEM_COMMAND(
      ...
      "pid,psr,cpu,cmd > %s; fi; rm %s/_psOKlock &> /dev/null; }",
      ...
      FILE *fpout;
      {
        int slen =
            snprintf(command, STRINGMAXLEN_COMMAND,
                     ...
                     "\"^[[:space:]][[:digit:]]+[[:space:]]+${CORENUM}\"|wc -l",
                     cpu, psoutfname);
        if (slen < 1) {
          PRINT_ERROR("snprintf wrote <1 char");
          abort();  // can't handle this error any other way
...
static int PIDcollectSystemInfo(PROCESSINFODISP *pinfodisp, int level) {
  ...
  for (int spindex = 0; spindex < pinfodisp->NBsubprocesses; spindex++) {
    ...
    int stat_tpgid;    // (8) The ID of the foreground process group of the
                       // controlling terminal of the process
    unsigned int stat_flags;  // (9) The kernel flags word of the process
    unsigned long
        stat_minflt;  // (10) The number of minor faults the process has made
                      // which have not required loading a memory page from disk
    unsigned long stat_cminflt;  // (11) The number of minor faults that the
                                 // process's waited-for children have made
    unsigned long
        stat_majflt;  // (12) The number of major faults the process has made
                      // which have required loading a memory page from disk
    unsigned long stat_cmajflt;  // (13) The number of major faults that the
                                 // process's waited-for children have made
    unsigned long stat_utime;  // (14) Amount of time that this process has been
                               // scheduled in user mode, measured in clock
                               // ticks (divide by sysconf(_SC_CLK_TCK)).
    unsigned long
        stat_stime;  // (15) Amount of time that this process has been scheduled
                     // in kernel mode, measured in clock ticks
    long stat_cutime;  // (16) Amount of time that this process's waited-for
                       // children have been scheduled in user mode, measured in
                       // clock ticks
    long stat_cstime;  // (17) Amount of time that this process's waited-for
                       // children have been scheduled in kernel mode, measured
                       // in clock ticks
    long stat_priority;  // (18) (Explanation for Linux 2.6) For processes
                         // running a
    ...
    long stat_nice;  // (19) The nice value (see setpriority(2)), a value in the
                     // range 19 (low priority) to -20 (high priority)
    long stat_num_threads;  // (20) Number of threads in this process
    long stat_itrealvalue;  // (21) hard coded as 0
    unsigned long long stat_starttime;  // (22) The time the process started
                                        // after system boot in clock ticks
    unsigned long stat_vsize;           // (23)  Virtual memory size in bytes
    long stat_rss;  // (24) Resident Set Size: number of pages the process has
                    // in real memory
    unsigned long stat_rsslim;  // (25) Current soft limit in bytes on the rss
                                // of the process
    unsigned long
    ...
    unsigned long stat_startstack;  // (28) The address of the start (i.e.,
                                    // bottom) of the stack
    unsigned long
        stat_kstkesp;  // (29) The current value of ESP (stack pointer), as
                       // found in the kernel stack page for the process
    unsigned long stat_kstkeip;  // (30) The current EIP (instruction pointer)
    unsigned long stat_signal;  // (31) The bitmap of pending signals, displayed
                                // as a decimal number.  Obsolete, because it
                                // does not provide information on real-time
                                // signals; use /proc/[pid]/status instead
    unsigned long
        stat_blocked;  // (32) The bitmap of blocked signals, displayed as a
                       // decimal number.  Obsolete, because it does not provide
                       // information on real-time signals; use
                       // /proc/[pid]/status instead.
    unsigned long
        stat_sigignore;  // (33) The bitmap of ignored signals, displayed as a
                         // decimal number.  Obsolete, because it does not
                         // provide information on real-time signals; use
                         // /proc/[pid]/status instead.
    unsigned long
        stat_sigcatch;  // (34) The bitmap of ignored signals, displayed as a
                        // decimal number.  Obsolete, because it does not
                        // provide information on real-time signals; use
                        // /proc/[pid]/status instead.
    unsigned long
        stat_wchan;  // (35) This is the "channel" in which the process is
                     // waiting.  It is the address of a location in the kernel
    ...
    unsigned long stat_cnswap;  // (37) Cumulative nswap for child processes
                                // (not maintained)
    int stat_exit_signal;       // (38) Signal to be sent to parent when we die
    int stat_processor;         // (39) CPU number last executed on
    unsigned int
        stat_rt_priority;  // (40) Real-time scheduling priority, a number in
    ...
    unsigned long long
        stat_delayacct_blkio_ticks;  // (42) Aggregated block I/O delays,
                                     // measured in clock ticks
    unsigned long
        stat_guest_time;  // (43) Guest time of the process (time spent running
                          // a virtual CPU for a guest operating system),
                          // measured in clock ticks
    long stat_cguest_time;  // (44) Guest time of the process's children,
                            // measured in clock ticks (divide by
                            // sysconf(_SC_CLK_TCK)).
    unsigned long
        stat_start_data;  // (45) Address above which program initialized and
                          // uninitialized (BSS) data are placed
    unsigned long stat_end_data;  // (46) ddress below which program initialized
                                  // and uninitialized (BSS) data are placed
    unsigned long stat_start_brk;  // (47) Address above which program heap can
                                   // be expanded with brk(2)
    unsigned long stat_arg_start;  // (48) Address above which program
                                   // command-line arguments (argv) are placed
    unsigned long stat_arg_end;    // (49) Address below program command-line
                                   // arguments (argv) are placed
    unsigned long stat_env_start;  // (50) Address above which program
                                   // environment is placed
    unsigned long
        stat_env_end;  // (51) Address below which program environment is placed
    long stat_exit_code;  // (52) The thread's exit status in the form reported
                          // by waitpid(2)
    ...
    Nfields = fscanf(
        ...
        "%lu %d %d %u %u %llu %lu %ld %lu %lu %lu %lu %lu %lu %lu %ld\n",
        &stat_pid,  //  1
        stat_comm, &stat_state, &stat_ppid, &stat_pgrp, &stat_session,
        &stat_tty_nr, &stat_tpgid, &stat_flags,
        &stat_minflt,  //  10
        &stat_cminflt, &stat_majflt, &stat_cmajflt, &stat_utime, &stat_stime,
        &stat_cutime, &stat_cstime, &stat_priority, &stat_nice,
        &stat_num_threads,  // 20
        &stat_itrealvalue, &stat_starttime, &stat_vsize, &stat_rss,
        &stat_rsslim, &stat_startcode, &stat_endcode, &stat_startstack,
        &stat_kstkesp,
        &stat_kstkeip,  // 30
        &stat_signal, &stat_blocked, &stat_sigignore, &stat_sigcatch,
        &stat_wchan, &stat_nswap, &stat_cnswap, &stat_exit_signal,
        &stat_processor,
        &stat_rt_priority,  // 40
        &stat_policy, &stat_delayacct_blkio_ticks, &stat_guest_time,
        &stat_cguest_time, &stat_start_data, &stat_end_data, &stat_start_brk,
        &stat_arg_start, &stat_arg_end,
        &stat_env_start,  // 50
        &stat_env_end, &stat_exit_code);
    if (Nfields != 52) {
      PRINT_ERROR("fscanf returns value != 1");
...
int processinfo_CPUsets_List(STRINGLISTENTRY *CPUsetList) {
  ...
  EXECUTE_SYSTEM_COMMAND(
      "cset set -l | awk '/root/{stop=1} stop==1{print $0}' > _tmplist.txt");
  ...
  while (pinfop->loop == 1) {
    ...
    for (pindex = 0; pindex < pinfop->NBpinfodisp; pindex++) {
      if (pinfop->loop == 1) {
        ...
        //    if(pinfolist->active[pindex] == 2) // mmap crashed, file may still
        //    be present
        ...
        if (pinfolist->active[pindex] == 1) {
          ...
          WRITE_FULLFILENAME(procfname, "/proc/%d",
                             (int)pinfolist->PIDarray[pindex]);
        ...
        if ((pindex < pinfop->NBpinfodisp) &&
          ...
          pinfop->pinfoarray[pindex] =
              processinfo_shm_link(SM_fname, &pinfop->fdarray[pindex]);
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
            strncpy(pinfop->pinfodisp[pindex].name,
                    pinfop->pinfoarray[pindex]->name, 40 - 1);
#pragma GCC diagnostic pop
          ...
          // pinfop->updatearray[pindex] == 0; // by default, no need to
          // re-connect
        ...
        if (pinfop->loop == 1) {
          ...
          if (pinfolist->active[pindexdisp] != 0) {
            ...
            if (pinfop->pinfodisp[pindexdisp].NBsubprocesses !=
                0)  // pinfop->pinfodisp[pindex].NBsubprocesses should never be
                    // zero - should be at least 1 (for main process)
            {
              int spindex;  // sub process index, 0 for main

              if (pinfop->psysinfostatus[pindexdisp] != -1) {
                for (spindex = 0;
                     spindex < pinfop->pinfodisp[pindexdisp].NBsubprocesses;
                     spindex++) {
              ...
              pinfop->psysinfostatus[pindex] =
                  PIDcollectSystemInfo(&(pinfop->pinfodisp[pindexdisp]), 0);
                ...
                for (spindex = 0;
                  ...
                  if (pinfop->pinfodisp[pindexdisp].sampletimearray[spindex] !=
                    ...
                    pinfop->pinfodisp[pindexdisp].subprocCPUloadarray[spindex] =
                        100.0 *
                        ((1.0 * pinfop->pinfodisp[pindexdisp]
                    ...
                    pinfop->pinfodisp[pindexdisp]
                        .subprocCPUloadarray_timeaveraged[spindex] =
                        0.9 * pinfop->pinfodisp[pindexdisp]
                                  .subprocCPUloadarray_timeaveraged[spindex] +
...
errno_t processinfo_CTRLscreen() {
  ...
  PROCINFOPROC
  procinfoproc;  // Main structure - holds everything that needs to be shared
                 // with other functions and scan thread
  ...
  */
  ...
  procinfoproc.pinfodisp = (PROCESSINFODISP *)malloc(sizeof(PROCESSINFODISP) *
  ...
  for (pindex = 0; pindex < procinfoproc.NBpinfodisp; pindex++) {
    ...
    for (int spi = 0; spi < MAXNBSUBPROCESS; spi++) {
      ...
      procinfoproc.pinfodisp[pindex].subprocCPUloadarray_timeaveraged[spi] =
          0.0;
  ...
  while (loopOK == 1) {
    ...
    //		else
    //			system("echo \"scanblock request read 0\" >
    // steplog.dQRr0.txt");//TEST
    ...
    switch (ch) {
      ...
      case ' ':  // Mark current PID as selected (if none selected, other
                 // commands only apply to highlighted process)
      ...
      case 't':
        TUI_exit();
        EXECUTE_SYSTEM_COMMAND(
            "tmux a -t %s", procinfoproc.pinfoarray[pindexSelected]->tmuxname);
        TUI_init_terminal(&wrow, &wcol);
        break;
      ...
      case 'L':  // toggle time limit (iter)
        ...
        if (ToggleValue == 0) {
          ...
          procinfoproc.pinfoarray[pindex]->dtiter_limit_value =
              (long)(1.5 * procinfoproc.pinfoarray[pindex]->dtmedian_iter_ns);
      ...
      case 'M':  // toggle time limit (exec)
        ...
        if (ToggleValue == 0) {
          ...
          procinfoproc.pinfoarray[pindex]->dtexec_limit_value =
              (long)(1.5 * procinfoproc.pinfoarray[pindex]->dtmedian_exec_ns +
                     20000);
    ...
    if (freeze == 0) {
      ...
      if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_HELP) {
        ...
        attroff(attrval);
        TUI_printfw(
            "   htop (F10 to exit) / iotop (q to exit) / atop (q to exit)");
        TUI_newline();
        ...
        attroff(attrval);
        TUI_printfw(
            "    Increase/decrease display frequency, (f)reeze display");
        TUI_newline();
        ...
        attroff(attrval);
        TUI_printfw(
            "    Remove selected (r) or all (R) inactive process(es) log");
        TUI_newline();
        ...
        attroff(attrval);
        TUI_printfw(
            "    (p)ause (toggle C0 - C1); (c)ompute on/off (toggle C0 - C5); "
            "(s)tep");
        TUI_newline();
        ...
        TUI_printfw("pindexSelected = %d    %d", pindexSelected,
                    pindexSelectedOK);
        TUI_newline();

        TUI_printfw(
            "[PID %d   SCAN TID %d]  %2d cpus   %2d processes tracked    "
            "Display Mode %d",
        ...
        TUI_printfw(
            "Display frequ = %2d Hz  [%ld] fscan=%5.2f Hz ( %5.2f Hz %5.2f %% "
            "busy )",
            (int)(frequ + 0.5), procinfoproc.loopcnt, 1.0 / procinfoproc.dtscan,
        ...
        if ((pindexSelected >= 0) && (pindexSelected < PROCESSINFOLISTSIZE)) {
          if (procinfoproc.pinfommapped[pindexSelected] == 1) {
            strcpy(pselected_FILE,
                   procinfoproc.pinfoarray[pindexSelected]->source_FILE);
            strcpy(pselected_FUNCTION,
                   procinfoproc.pinfoarray[pindexSelected]->source_FUNCTION);
            pselected_LINE =
                procinfoproc.pinfoarray[pindexSelected]->source_LINE;

            TUI_printfw("Source Code: %s line %d (function %s)", pselected_FILE,
        ...
        if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_RESOURCES) {
          ...
          // List CPUs
          TUI_printfw(" %*.*s %*.*s %-*.*s    %-*.*s              ",
                      pstrlen_status, pstrlen_status, " ", pstrlen_pid,
                      pstrlen_pid, " ", pstrlen_pname, pstrlen_pname, " ",
          ...
          TUI_printfw(" <- %2d sockets %2d CPUs", procinfoproc.NBcpusocket,
                      procinfoproc.NBcpus);
          TUI_newline();
          ...
          // Print CPU LOAD
          TUI_printfw(" %*.*s %*.*s %-*.*s PR %-*.*s  #T  ctxsw   ",
                      pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid,
                      pstrlen_pid, "PID", pstrlen_pname, pstrlen_pname, "pname",
                      pstrlen_cset, pstrlen_cset, "cset", procinfoproc.NBcpus);
          ...
          for (cpusocket = 0; cpusocket < procinfoproc.NBcpusocket;
            ...
            for (cpu = 0; cpu < procinfoproc.NBcpus; cpu++)
              if (procinfoproc.CPUphys[cpu] == cpusocket) {
                int vint =
                    (int)(100.0 *
                          procinfoproc.CPUload[procinfoproc.CPUids[cpu]]);
                if (vint > 99) {
                  vint = 99;
                }
        ...
        if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL) {
          ...
          TUI_newline();
          TUI_printfw(
              " %*.*s %*.*s %-*.*s %-*.*s C# tstart       %-*.*s %-*.*s   "
              "%-*.*s   %-*.*s",
              pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid,
              pstrlen_pid, "PID", pstrlen_pname, pstrlen_pname, "pname",
              pstrlen_state, pstrlen_state, "state", pstrlen_tmux, pstrlen_tmux,
              "tmux sess", pstrlen_loopcnt, pstrlen_loopcnt, "loopcnt",
              pstrlen_descr, pstrlen_descr, "Description", pstrlen_msg,
              pstrlen_msg, "Message");
          TUI_newline();
          TUI_newline();
          DEBUG_TRACEPOINT(" ");
        ...
        if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TRIGGER) {
          ...
          TUI_newline();
          TUI_printfw(
              "%*.*s %*.*s %-*.*s %*.*s %*.*s mode sem %*.*s  %*.*s  %*.*s",
              pstrlen_status, pstrlen_status, "STATUS", pstrlen_pid,
              pstrlen_pid, "PID", pstrlen_pname, pstrlen_pname, "pname",
              pstrlen_inode, pstrlen_inode, "inode", pstrlen_trigstreamname,
              pstrlen_trigstreamname, "stream", pstrlen_missedfr,
              pstrlen_missedfr, "miss", pstrlen_missedfrc, pstrlen_missedfrc,
              "misscumul", pstrlen_tocnt, pstrlen_tocnt, "timeouts");
          TUI_newline();
          TUI_newline();
          DEBUG_TRACEPOINT(" ");
        }
        ...
        if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TIMING) {
          ...
          TUI_newline();
          TUI_newline();
          TUI_printfw("   STATUS    PID   process name                    ");
          TUI_newline();
          TUI_newline();
          DEBUG_TRACEPOINT(" ");
        }
        ...
        for (dispindex = 0; dispindex < dispindexMax; dispindex++) {
          ...
          if (pindex < procinfoproc.NBpinfodisp) {
            ...
            if (pinfolist->active[pindex] == 1) {
              sprintf(string, "%-*.*s", pstrlen_status, pstrlen_status,
                      "ACTIVE");
              attron(COLOR_PAIR(2));
            ...
            DEBUG_TRACEPOINT("pinfolist->active[pindex] = %d",
            ...
            if (pinfolist->active[pindex] != 0) {
              ...
              if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_CTRL) {
                ...
                if (procinfoproc.pinfoarray[pindex]->CTRLval == 0) {
                  attron(COLOR_PAIR(2));
                  TUI_printfw(" C%d", procinfoproc.pinfoarray[pindex]->CTRLval);
                  attroff(COLOR_PAIR(2));
                } else {
                  TUI_printfw(" C%d", procinfoproc.pinfoarray[pindex]->CTRLval);
                }
                ...
                sprintf(string, " %02d:%02d:%02d.%03d",
                        ...
                        (int)(0.000001 *
                              (procinfoproc.pinfodisp[pindex].createtime_ns)));
                TUI_printfw("%s", string);
                ...
                if (procinfoproc.psysinfostatus[pindex] == -1) {
                  ...
                  for (spindex = 0;
                    ...
                    if (spindex > 0) {
                      ...
                      sprintf(string, " %*.*s %-*.*d %-*.*s", pstrlen_status,
                              pstrlen_status, "|---", pstrlen_pid, pstrlen_pid,
                    ...
                    sprintf(string, " %2d",
                            procinfoproc.pinfodisp[pindex].rt_priority);
                    TUI_printfw("%s", string);
                    ...
                    sprintf(string, " %2dx ",
                            procinfoproc.pinfodisp[pindex].threads);
                    TUI_printfw("%s", string);
                    ...
                    cpuColor = 1;
                    if (procinfoproc.pinfodisp[pindex]
                            .subprocCPUloadarray_timeaveraged[spindex] > 10.0) {
                      cpuColor = 2;
                    }
                    if (procinfoproc.pinfodisp[pindex]
                            .subprocCPUloadarray_timeaveraged[spindex] > 20.0) {
                      cpuColor = 3;
                    }
                    if (procinfoproc.pinfodisp[pindex]
                            .subprocCPUloadarray_timeaveraged[spindex] > 40.0) {
                      cpuColor = 4;
                    }
                    if (procinfoproc.pinfodisp[pindex]
                            .subprocCPUloadarray_timeaveraged[spindex] < 1.0) {
                      cpuColor = 5;
                    }

                    // First group of cores (physical CPU 0)
                    for (cpu = 0;
                         cpu < procinfoproc.NBcpus / procinfoproc.NBcpusocket;
                         cpu++) {
                      TUI_printfw("|");
                      ...
                      if (procinfoproc.pinfodisp[pindex].cpuOKarray[cpu] == 1) {
                        TUI_printfw("%2d", procinfoproc.CPUids[cpu]);
                      } else {
                        TUI_printfw("  ");
                      }
                  ...
                  if (procinfoproc.pinfodisp[pindex].NBsubprocesses == 0) {
                    TUI_printfw(
                        "  ERROR: "
                        "procinfoproc.pinfodisp[pindex].NBsubprocesses = %d",
              ...
              if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TRIGGER) {
                ...
                TUI_printfw("%*s ", pstrlen_trigstreamname,
              ...
              if (procinfoproc.DisplayMode == PROCCTRL_DISPLAYMODE_TIMING) {
                ...
                if (procinfoproc.pinfoarray[pindex]->MeasureTiming == 1) {
                  ...
                  TUI_printfw(
                      " %3d ..%02ld  ",
                      procinfoproc.pinfoarray[pindex]->timerindex,
                      procinfoproc.pinfoarray[pindex]->timingbuffercnt % 100);

                  // compute timing stat
                  dtiter_array =
                      (long *)malloc(sizeof(long) * (PROCESSINFO_NBtimer - 1));
                    ...
                    dtiter_array[tindex] =
                        (procinfoproc.pinfoarray[pindex]
                         ...
                         procinfoproc.pinfoarray[pindex]
                             ->texecstart[ti0]
                             .tv_nsec) +
                        1000000000 * (procinfoproc.pinfoarray[pindex]
                                          ->texecstart[ti1]
                                          .tv_sec -
                                      procinfoproc.pinfoarray[pindex]
                                          ->texecstart[ti0]
                  ...
                  if (procinfoproc.pinfoarray[pindex]->dtiter_limit_enable !=
                      0) {
                    if (procinfoproc.pinfoarray[pindex]->dtiter_limit_cnt ==
                        0) {
                  ...
                  TUI_printfw(
                      ...
                      (long)(0.001 * procinfoproc.pinfoarray[pindex]
                                         ->dtiter_limit_value),
                  ...
                  if (procinfoproc.pinfoarray[pindex]->dtexec_limit_enable !=
                      0) {
                    if (procinfoproc.pinfoarray[pindex]->dtexec_limit_cnt ==
                        0) {
                      colorcode = COLOR_PAIR(2);
                    } else {
                      colorcode = COLOR_PAIR(4);
                  ...
                  procinfoproc.pinfoarray[pindex]->dtmedian_iter_ns =
                      dtiter_array[(long)(0.5 * PROCESSINFO_NBtimer)];
                  if (tval > 9999.9) {
                  ...
                  procinfoproc.pinfoarray[pindex]->dtmedian_exec_ns =
                      dtexec_array[(long)(0.5 * PROCESSINFO_NBtimer)];
                  if (tval > 9999.9) {
                  ...
                  TUI_printfw(
                      "  busy = %6.2f %%",
                      100.0 * dtexec_array[(long)(0.5 * PROCESSINFO_NBtimer)] /
                          (dtiter_array[(long)(0.5 * PROCESSINFO_NBtimer)] +
                           1));
      ...
      TUI_newline();
      TUI_printfw("Loop time = %9.8f s  ( max rate = %7.2f Hz)", tdiffvloop,
                  1.0 / tdiffvloop);
      TUI_newline();
