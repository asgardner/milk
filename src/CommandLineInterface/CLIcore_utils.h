...
#define CLICMD_FIELDS_NOFPS \
  __FILE__, sizeof(farg) / sizeof(CLICMDARGDEF), farg, 0, NULL
...
#define FARG_OUTIM2D(imkey)                                         \
  {CLIARG_STR, "." #imkey ".name",     "output image",              \
   #imkey,     CLIARG_VISIBLE_DEFAULT, (void **)&imkey.name},       \
      {CLIARG_LONG, "." #imkey ".xsize",    "x size",               \
       "512",       CLIARG_VISIBLE_DEFAULT, (void **)&imkey.xsize}, \
      {CLIARG_LONG, "." #imkey ".ysize",    "y size",               \
       "512",       CLIARG_VISIBLE_DEFAULT, (void **)&imkey.ysize}, \
      {CLIARG_LONG, "." #imkey ".shared",  "shared flag",           \
       "0",         CLIARG_HIDDEN_DEFAULT, (void **)&imkey.shared}, \
      {CLIARG_LONG, "." #imkey ".NBkw",    "number keywords",       \
       "10",        CLIARG_HIDDEN_DEFAULT, (void **)&imkey.NBkw},   \
  {                                                                 \
    CLIARG_LONG, "." #imkey ".CBsize", "circ buffer size", "0",     \
        CLIARG_HIDDEN_DEFAULT, (void **)&imkey.CBsize               \
  }
...
#define INSERT_STD_FPSCONFfunction                                  \
  static errno_t FPSCONFfunction() {                                \
    ...
    STD_FARG_LINKfunction FPS_CONFLOOP_START data.fpsptr = NULL;    \
    FPS_CONFLOOP_END                                                \
    return RETURN_SUCCESS;                                          \
  }
...
#define INSERT_STD_PROCINFO_COMPUTEFUNC_START                                \
  ...
  if (data.fpsptr !=                                                         \
      NULL) { /* If FPS mode, then FPS settings override defaults*/          \
  ...
  if (CLIcmddata.cmdsettings->flags & CLICMDFLAG_PROCINFO) {                 \
    ...
    if (data.fpsptr != NULL) {                                               \
      processinfo = processinfo_setup(data.FPS_name, pinfodescr, "startup",  \
                                      __FUNCTION__, __FILE__, __LINE__);     \
      fps_to_processinfo(data.fpsptr, processinfo);                          \
    } else {                                                                 \
      processinfo = processinfo_setup(CLIcmddata.key, pinfodescr, "startup", \
                                      __FUNCTION__, __FILE__, __LINE__);     \
    }                                                                        \
    ...
    FUNC_CHECK_RETURN(processinfo_waitoninputstream_init(                    \
        processinfo, processinfo->triggerstreamID,                           \
  ...
  if (target_img->ID == -1) {
    if (target_img != source_img) {
      printf("Creating image %s from %s, shared = %d, kw = %d\n",
             target_img->name, source_img->name, source_img->shared,
             source_img->NBkw);
    } else {
    ...
    create_image_ID(target_img->name, source_img->naxis, source_img->size,
                    source_img->datatype, source_img->shared, source_img->NBkw,
                    source_img->CBsize, &target_img->ID);
    DEBUG_TRACEPOINT(" ");
