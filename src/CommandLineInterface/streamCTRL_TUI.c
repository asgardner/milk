...
#define SHAREDSHMDIR                                                  \
  data.shmdir /**< default location of file mapped semaphores, can be \
...
static int get_PIDmax() {
  ...
  if (fscanfcnt == EOF) {
    if (ferror(fp)) {
      ...
      fprintf(stderr,
              "Error: fscanf reached end of file, no matching characters, no "
              "matching failure\n");
    }
    exit(EXIT_FAILURE);
  } else if (fscanfcnt != 1) {
    fprintf(stderr,
            "Error: fscanf successfully matched and assigned %i input items, 1 "
            "expected\n",
            fscanfcnt);
    exit(EXIT_FAILURE);
  }
  ...
  if (d) {
    ...
    while (((dir = readdir(d)) != NULL)) {
      ...
      if ((pch) && (matchOK == 1)) {
        ...
        WRITE_FULLFILENAME(fullname, "%s/%s", SHAREDSHMDIR, dir->d_name);
...
#pragma GCC diagnostic pop
          streaminfo[sindex].sname[strlen(dir->d_name) - strlen(".im.shm")] =
              '\0';
...
void *streamCTRL_scan(void *argptr) {
  ...
  struct streamCTRLarg_struct *streamCTRLdata =
      (struct streamCTRLarg_struct *)argptr;
  ...
  while (streaminfoproc->loop == 1) {
    ...
    {
      NBsindex = find_streams(streaminfo, streaminfoproc->filter,
        ...
        while (((dir = readdir(d)) != NULL)) {
          ...
          if ((pch) && (matchOK == 1)) {
            ...
            if (S_ISLNK(buf.st_mode))  // resolve link name
            {
              ...
              streaminfo[sindex].SymLink = 1;
              WRITE_FULLFILENAME(fullname, "%s/%s", SHAREDSHMDIR, dir->d_name);
              //                        readlink (fullname, linknamefull,
              //                        200-1);
              linknamefull = realpath(fullname, NULL);
              ...
              {
                if (streaminfoproc->WriteFlistToFile == 1) {
                  fprintf(fpfscan, " %s <-> LINK %s CANNOT BE READ -> off",
                          fullname, linknamefull);
                }
                scanentryOK = 0;
              } else {
                strcpy(linkname, basename(linknamefull));
        ...
        if (PReadMode == 0) {
          // popen option
          {
            int slen = snprintf(command, STRINGMAXLEN_COMMAND,
                                "/bin/fuser %s/%s.im.shm 2>/dev/null",
          ...
          {
            int slen = snprintf(command, STRINGMAXLEN_COMMAND,
                                "/bin/fuser %s/%s.im.shm 2>/dev/null > %s",
                                SHAREDSHMDIR, streaminfo[sindexscan1].sname,
                                plistfname);
            if (slen < 1) {
              PRINT_ERROR("snprintf wrote <1 char");
...
static int streamCTRL_print_inode(ino_t inode, ino_t *upstreaminode,
                                  int NBupstreaminode, int downstreamindex) {
 ...
 *
 */
static int streamCTRL_print_procpid(int DispPID_NBchar, pid_t procpid,
                                    pid_t *upstreamproc, int NBupstreamproc,
                                    uint32_t mode) {
...
static errno_t streamCTRL_print_SPTRACE_details(IMAGE *streamCTRLimages,
  ...
  */
  ...
  TUI_newline();
  TUI_printfw("   %*s %*s %*s", Disp_inode_NBchar, "inode", Disp_sname_NBchar,
              "stream", Disp_cnt0_NBchar, "cnt0", Disp_PID_NBchar, "PID",
  ...
  for (int spti = 0; spti < streamCTRLimages[ID].md[0].NBproctrace; spti++) {
    ...
    Disp_PID_NBchar = streamCTRL_print_procpid(
        8, pid, upstreamproc, NBupstreamproc, PRINT_PID_FORCE_NOUPSTREAM);
    ...
    if (print_timing == 1) {
      ...
      tdiff = timespec_diff(
          streamCTRLimages[ID].streamproctrace[spti].ts_procstart, tnow);
  ...
  while (loopOK == 1) {
    ...
    switch (ch) {
      ...
      case '}':  // faster scan update
        streaminfoproc.twaitus =
            (int)(0.83333333333333333333 * streaminfoproc.twaitus);
    ...
    if (DisplayMode == DISPLAY_MODE_HELP)  // help
    {
      ...
      TUI_printfw(
          "PIDmax = %d    Update frequ = %2d Hz  fscan=%5.2f Hz ( %5.2f Hz "
          "%5.2f %% busy ) ",
          PIDmax, (int)(frequ + 0.5), 1.0 / streaminfoproc.dtscan,
      ...
      if (streaminfoproc.fuserUpdate == 1) {
        ...
        TUI_printfw("fuser scan ongoing  %4d  / %4d   ",
                    streaminfoproc.sindexscan, NBsindex);
      ...
      if (DisplayMode == DISPLAY_MODE_FUSER) {
        if (fuserScan == 1) {
          TUI_printfw(
              "Last scan on  %02d:%02d:%02d  - Press F6 again to re-scan    "
              "C-c to stop scan",
              uttime_lastScan->tm_hour, uttime_lastScan->tm_min,
              uttime_lastScan->tm_sec);
          TUI_newline();
        } else {
          TUI_printfw(
              "Last scan on  XX:XX:XX  - Press F6 again to scan             "
              "C-c to stop scan");
          TUI_newline();
        }
      } else {
      ...
      TUI_printfw(
          "%4d streams    Currently displaying %4d-%4d   Selected %d  ID = %d  "
          "inode = %d",
          NBsindex, doffsetindex, lastindex, dindexSelected,
          ssindex[dindexSelected], (int)inodeselected);
      ...
      TUI_printfw("%*s  %-*s  %-*s  %*s   %*s %*s %*s %8s", 9, "inode",
                  DispName_NBchar, "name", DispSize_NBchar, "type",
                  Dispcnt0_NBchar, "cnt0", DispPID_NBchar, "creaPID",
                  DispPID_NBchar, "ownPID", Dispfreq_NBchar, "   frequ ",
                  "#sem");
      ...
      switch (DisplayMode) {
        ...
        case DISPLAY_MODE_SPTRACE:
          TUI_printfw(
              "     stream process traces:   \"(INODE TYPE/SEM PID)>\"");
          TUI_newline();
          break;
      ...
      for (dindex = 0; dindex < NBsindex; dindex++) {
        ...
        if (dindex == dindexSelected) {
          ...
          NBupstreaminode = 0;
          for (int spti = 0; spti < streamCTRLimages[ID].md[0].NBproctrace;
               spti++) {
            if (NBupstreaminode < NBupstreaminodeMAX) {
              ino_t inode =
                  streamCTRLimages[ID].streamproctrace[spti].trigger_inode;
              if (inode != 0) {
        ...
        if (DisplayFlag == 1) {
          if (streaminfo[sindex].SymLink == 1) {
            ...
            snprintf(namestring, stringmaxlen, "%s->%s",
        ...
        if ((DisplayMode < DISPLAY_MODE_FUSER) && (DisplayFlag == 1)) {
          ...
          if (streamCTRLimages[streaminfo[sindex].ID].md == NULL) {
            ...
            sprintf(string, " %*ld ", Dispcnt0_NBchar,
          ...
          if (streamCTRLimages[streaminfo[sindex].ID].md == NULL) {
            ...
            streamCTRL_print_procpid(8, cpid, upstreamproc, NBupstreamproc,
                                     print_pid_mode);
            TUI_printfw(" ");
            streamCTRL_print_procpid(8, opid, upstreamproc, NBupstreamproc,
                                     print_pid_mode);
            TUI_printfw(" ");
          }
          ...
          if (streamCTRLimages[streaminfo[sindex].ID].md == NULL) {
            ...
            sprintf(string, " %*.2f Hz", Dispfreq_NBchar,
                    streaminfo[sindex].updatevalue);
          }
          TUI_printfw(string);
        }
        ...
        if (streamCTRLimages[streaminfo[sindex].ID].md != NULL) {
              ...
              (DisplayFlag == 1))  // sem read PIDs
          {
            ...
            if (DisplayDetailLevel == 1) {
              TUI_newline();
              streamCTRL_print_SPTRACE_details(streamCTRLimages, ID,
                                               upstreamproc, NBupstreamproc,
                                               PRINT_PID_DEFAULT);
            }
          }
        }
          ...
          switch (streaminfo[sindex].streamOpenPID_status) {
            case 1:
              ...
              for (pidIndex = 0;
                ...
                if ((getpgid(pid) >= 0) && (pid != getpid())) {
                  sprintf(string, ":%-*.*s", PIDnameStringLen, PIDnameStringLen,
                          PIDname_array[pid]);
                  TUI_printfw(string);
