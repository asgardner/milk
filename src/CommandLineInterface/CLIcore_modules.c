...
errno_t load_module_shared(const char *__restrict modulename) {
  ...
  // printf("Searching for shared object in directory MILK_INSTALLDIR/lib :
  // %s/lib\n", getenv("MILK_INSTALLDIR"));
  DEBUG_TRACEPOINT(
      "Searching for shared object in directory [data.installdir]/lib : %s/lib",
      data.installdir);
  ...
  // this variable will be written by module register function into module
  // struct
    ...
    if (d) {
      while ((dir = readdir(d)) != NULL) {
        ...
        if (dot && !strcmp(dot, ".so")) {
          WRITE_FULLFILENAME(libname, "%s/lib/%s", data.installdir,
                             dir->d_name);
          // printf("%02d   (re-?) LOADING shared object  %40s -> %s\n",
          // DLib_index, dir->d_name, libname); fflush(stdout);
...
errno_t RegisterModule(const char *__restrict FileName,
                       ...
                       const char *__restrict InfoString, int versionmajor,
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
  strncpy(data.module[data.NBmodule].loadname, data.moduleloadname,
          STRINGMAXLEN_MODULE_LOADNAME - 2);
...
#pragma GCC diagnostic pop
  ...
  if (data.progStatus == 0) {
    ...
    // PackageName, FileName); 	fflush(stdout);
  }

  if (data.progStatus == 1) {
    OKmsg = 1;
    DEBUG_TRACEPOINT(
        "  %02ld  Found unloaded shared object in ./libs/ -> LOADING %10s  "
        "module %40s",
...
#define MIN(a, b) (a > b ? b : a)
      ...
      strncat(data.cmd[data.NBcmd].key, CLIkey,
              MIN(strlen(CLIkey), sizeof(data.cmd[data.NBcmd].key) -
                                      strlen(data.cmd[data.NBcmd].key)));
    }
  }
...
#define MIN(a, b) (a > b ? b : a)
      ...
      strncat(data.cmd[data.NBcmd].key, ".", 2);
      strncat(
          data.cmd[data.NBcmd].key, CLIcmddata.key,
          MIN(strlen(CLIcmddata.key), sizeof(data.cmd[data.NBcmd].key) -
                                          strlen(data.cmd[data.NBcmd].key)));
      // otherwise, construct call key as <shortname>.<CLIkey>
      int slen = strlen(data.cmd[data.NBcmd].key);
      if (slen < 1) {
  ...
  CLIhelp_make_cmdexamplestring(CLIcmddata.funcfpscliarg, CLIcmddata.nbarg,
                                CLIcmddata.key, cmdexamplestring);
  ...
  if (CLIcmddata.nbarg > 0) {
    ...
    for (int argi = 0; argi < CLIcmddata.nbarg; argi++) {
      ...
      switch (data.cmd[data.NBcmd].argdata[argi].type) {
        case CLIARG_FLOAT:
          data.cmd[data.NBcmd].argdata[argi].val.f =
              atof(CLIcmddata.funcfpscliarg[argi].example);
          break;
        case CLIARG_LONG:
          data.cmd[data.NBcmd].argdata[argi].val.l =
              atol(CLIcmddata.funcfpscliarg[argi].example);
          break;
