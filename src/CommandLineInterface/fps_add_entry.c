...
errno_t function_parameter_add_entry(FUNCTION_PARAMETER_STRUCT *fps,
                                     ...
                                     uint64_t type, uint64_t fpflag,
                                     void *valueptr, long *pindexptr) {
...
#pragma GCC diagnostic pop
    ...
    while (pch != NULL) {
      strncpy(
          funcparamarray[pindex].keyword[funcparamarray[pindex].keywordlevel],
    ...
    switch (funcparamarray[pindex].type) {
      ...
      case FPTYPE_FILENAME:
        if (snprintf(funcparamarray[pindex].val.string[0],
                     FUNCTION_PARAMETER_STRMAXLEN, "NULL") < 0) {
          PRINT_ERROR("snprintf error");
        }
        if (snprintf(funcparamarray[pindex].val.string[1],
                     FUNCTION_PARAMETER_STRMAXLEN, "NULL") < 0) {
          PRINT_ERROR("snprintf error");
        }
        break;
      ...
      case FPTYPE_ONOFF:
        ...
        if (snprintf(funcparamarray[pindex].val.string[0],
                     FUNCTION_PARAMETER_STRMAXLEN, "OFF state") < 0) {
          PRINT_ERROR("snprintf error");
        }
        if (snprintf(funcparamarray[pindex].val.string[1],
                     FUNCTION_PARAMETER_STRMAXLEN, " ON state") < 0) {
          PRINT_ERROR("snprintf error");
        }
        break;
  ...
  /*
      ...
      for(index = 0; index < 3; index++)
      {
          ...
          {
              case 0 :
                  functionparameter_GetFileName(fps, &funcparamarray[pindex],
     fname, "setval"); break;
     ...
     fname, "maxval"); break;
          ...
          if((fp = fopen(fname, "r")) != NULL)
          {
              switch(funcparamarray[pindex].type)
              {
                  ...
                  case FPTYPE_INT64 :
                      if(fscanf(fp, "%ld", &funcparamarray[pindex].val.l[index])
     == 1) if(index == 0)     // return value is set by setval, cnt0 tracks
     updates to setval, not to minval or maxval
                          {
                              RVAL = 1;
                              funcparamarray[pindex].cnt0++;
                          }
                  ...
                  case FPTYPE_FLOAT64 :
                      if(fscanf(fp, "%lf", &funcparamarray[pindex].val.f[index])
     == 1) if(index == 0)
                          {
                              RVAL = 1;
                              funcparamarray[pindex].cnt0++;
                  ...
                  case FPTYPE_FLOAT32 :
                      if(fscanf(fp, "%f", &funcparamarray[pindex].val.s[index])
     == 1) if(index == 0)
                          {
                              RVAL = 1;
                              funcparamarray[pindex].cnt0++;
                  ...
                  case FPTYPE_PID :
                      ...
                      {
                          if(fscanf(fp, "%d",
     &funcparamarray[pindex].val.pid[index]) == 1)
                          {
                              RVAL = 1;
                          }
                          funcparamarray[pindex].cnt0++;
                  ...
                  case FPTYPE_TIMESPEC :
                      if(fscanf(fp, "%ld %ld",
     &funcparamarray[pindex].val.ts[index].tv_sec,
                                &funcparamarray[pindex].val.ts[index].tv_nsec)
     == 2) if(index == 0)
                          {
                              RVAL = 1;
                              funcparamarray[pindex].cnt0++;
                  ...
                  case FPTYPE_FILENAME :
                      ...
                      {
                          if(fscanf(fp, "%s",
     funcparamarray[pindex].val.string[0]) == 1)
                          {
                              RVAL = 1;
                              funcparamarray[pindex].cnt0++;
                          }
     ...
     funcparamarray[pindex].val.string[0]) == 1)
      ...
      if(RVAL != 0)
      {
          functionparameter_WriteParameterToDisk(fps, pindex, "fpsname",
     "AddEntry"); functionparameter_WriteParameterToDisk(fps, pindex, "fpsdir",
     "AddEntry"); functionparameter_WriteParameterToDisk(fps, pindex, "status",
     "AddEntry");
      }
  */
