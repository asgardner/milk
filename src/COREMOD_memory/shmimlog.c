...
static CLICMDARGDEF farg[] = {
    {CLIARG_IMG, ".in_sname", "input stream name", "im1",
     CLIARG_VISIBLE_DEFAULT, (void **)&instreamname},
    {CLIARG_LONG, ".cubesize", "cube size", "10000", CLIARG_VISIBLE_DEFAULT,
     (void **)&logcubesize},
    {CLIARG_STR, ".logdir", "log directory", "/media/data",
     CLIARG_VISIBLE_DEFAULT, (void **)&logdir}};

// flag CLICMDFLAG_FPS enabled FPS capability
static CLICMDDATA CLIcmddata = {"shmimlog", "log shared memory stream",
                                CLICMD_FIELDS_DEFAULTS};

// Forward declarations

static errno_t __attribute__((hot))
shmimlog2D(const char *IDname, uint32_t zsize, const char *logdir,
 ...
 * if an image name logdata exists (should ideally be in shared mem), then this
 * will be included in the timing txt file
 */
static errno_t __attribute__((hot))
shmimlog2D(const char *IDname, uint32_t zsize, const char *logdir,
           const char *IDlogdata_name) {
  ...
  while ((logshimconf[0].filecnt != NBfiles) && (logshimconf[0].logexit == 0)) {
    ...
    if (SkipWait == 1) {
      ...
      if (likely(use_semlog == 1)) {
        ...
        if (ret == -1) {
          ...
          if (errno == EINTR) {
            printf(
                "%5d  sem_timedwait [index %ld]: The call was interrupted by a "
                "signal handler\n",
                __LINE__, index);
          }
          ...
          if (errno == EINVAL) {
            printf("%5d  sem_timedwait [index %ld]: Not a valid semaphore\n",
                   __LINE__, index);
            printf(
                "               The value of abs_timeout.tv_nsecs is less than "
                "0, or greater than or equal to 1000 million\n");
          }

          if (errno == EAGAIN) {
            printf(
                "%5d  sem_timedwait [index %ld]: The operation could not be "
                "performed without blocking (i.e., the semaphore currently has "
                "the value zero)\n",
                __LINE__, index);
          }
        ...
        while (
            ((cnt == data.image[ID].md[0].cnt0) || (logshimconf[0].on == 0)) &&
            (wOK == 1)) {
          if (VERBOSE > 1) {
    ...
    if (index == 0) {
      ...
      //     uttime->tm_hour, uttime->tm_min, timenow.tv_sec % 60,
      //     timenow.tv_nsec);
      //            sprintf(fnameascii,"%s/%s_%02d:%02d:%02ld.%09ld.txt",
      //            logdir, IDname, uttime->tm_hour, uttime->tm_min,
      ...
      if (likely(wOK == 1))  // normal step: a frame has arrived
      {
        ...
        if (data.image[ID].md->CBsize > 0) {
          if (VERBOSE > 0) {
            ...
            printf("    LAST SAVED          [%8ld %3ld]\n",
                   logshimconf->CBcycle, (long)logshimconf->CBindex);
          }
          long currentCBcycle = data.image[ID].md->CBcycle;
          long currentCBindex = data.image[ID].md->CBindex;

          if (VERBOSE > 0) {
            printf("    LAST WRITTEN IN CB  [%8ld %3ld]\n",
                   (long)currentCBcycle, (long)currentCBindex);
          }

          long CBcyclediff = (long)currentCBcycle - (long)logshimconf->CBcycle;

          long CBindexdiff = (long)currentCBindex - (long)logshimconf->CBindex;
          ...
          if (VERBOSE > 0) {
            printf("    DIFF                [%8ld %3ld]    %8ld frames\n",
                   CBcyclediff, CBindexdiff, NBframesbehind);
          }
          ...
          if (VERBOSE > 0) {
            printf("    -> GRABBING %4ld frames (max %6ld) -> ", NBgrab,
                   grabmax_logbuff);
          }
          ...
          if (VERBOSE > 0) {
            printf("    CB BUFFER RANGE     %3ld -> %3ld\n", grabStart,
                   grabEnd);

            printf("     GRAB RANGE %3ld %3ld  (%3ld) ->  %6ld %6ld\n",
                   grabStart0, grabEnd0, grabSpan0, index0start, index0end);
            printf("     GRAB RANGE %3ld %3ld  (%3ld) ->  %6ld %6ld\n",
                   grabStart1, grabEnd1, grabSpan1, index1start, index1end);
          }
        ...
        if (data.image[ID].md->CBsize > 0)  // circ buffer mode
        {
          if (grabSpan0 > 0) {
            ...
            memcpy((void *)ptr1, (void *)ptr0, framesize * grabSpan0);
          }
          ...
          if (grabSpan1 > 0) {
            ...
            memcpy((void *)ptr1, (void *)ptr0, framesize * grabSpan1);
          }
          index += NBgrab;
        } else {
          // no circular buffer
          ...
          if (NBgrab > 0) {
            memcpy((void *)ptr1, (void *)ptr0, framesize * NBgrab);
    ...
    if ((index > zsize - 1) || ((timeout == 1) && (index > 0))) {
      ...
      sprintf(fnameascii, "%s/%s.%04d%02d%02dT%02d%02d%02ld.%09ldZ.txt", logdir,
              IDname, 1900 + uttimeStart->tm_year, 1 + uttimeStart->tm_mon,
              uttimeStart->tm_mday, uttimeStart->tm_hour, uttimeStart->tm_min,
      ...
      sprintf(fname, "%s/%s.%04d%02d%02dT%02d%02d%02ld.%09ldZ.fits", logdir,
              IDname, 1900 + uttimeStart->tm_year, 1 + uttimeStart->tm_mon,
              uttimeStart->tm_mday, uttimeStart->tm_hour, uttimeStart->tm_min,
              timenowStart.tv_sec % 60, timenowStart.tv_nsec);
      ...
      printf("=== %6ld %6ld  %6ld   %6ld ====\n", array_cnt0[0],
             array_cnt0[index - 1], index,
             array_cnt0[index - 1] - array_cnt0[0]);
      printf(
          "===== CUBE %8lld   Number of missed frames = %8ld  / %ld  / %8ld "
          "====\n",
