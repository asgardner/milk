...
static errno_t fps_create__cli() {
  if (0 + CLI_checkarg(1, CLIARG_LONG) + CLI_checkarg_noerrmsg(2, CLIARG_STR) ==
      0) {
...
errno_t fps_create_addCLIcmd() {
  RegisterCLIcommand("fpscreate", __FILE__, fps_create__cli,
                     ...
                     "<NBparam> <name>", "fpscreate 100 newfps",
...
errno_t function_parameter_struct_create(int NBparamMAX, const char *name) {
  ...
  fps.md = (FUNCTION_PARAMETER_STRUCT_MD *)mmap(
      0, sharedsize, PROT_READ | PROT_WRITE, MAP_SHARED, SM_fd, 0);
...
#pragma GCC diagnostic ignored "-Wstringop-truncation"
  ...
  strncpy(fps.md->callprogname, data.package_name,
