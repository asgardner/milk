...
errno_t COREMOD_MEMORY_image_streamburst(const char *IDin_name,
                                         const char *IDout_name, long periodus);

imageID COREMOD_MEMORY_image_streamupdateloop(const char *IDinname,
                                              const char *IDoutname,
                                              long usperiod, long NBcubes,
                                              long period, long offsetus,
                                              const char *IDsync_name,
                                              int semtrig, int timingmode);

imageID COREMOD_MEMORY_image_streamupdateloop_semtrig(
    const char *IDinname, const char *IDoutname, long period, long offsetus,
    const char *IDsync_name, int semtrig, int timingmode);
...
static errno_t COREMOD_MEMORY_image_streamupdateloop_semtrig__cli() {
  if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_STR) +
    ...
    COREMOD_MEMORY_image_streamupdateloop_semtrig(
        data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.string,
        data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.numl,
        data.cmdargtoken[5].val.string, data.cmdargtoken[6].val.numl,
...
errno_t stream_updateloop_addCLIcmd() {
  RegisterCLIcommand("streamburst", __FILE__,
  ...
  RegisterCLIcommand(
      "creaimstream", __FILE__, COREMOD_MEMORY_image_streamupdateloop__cli,
      "create 2D image stream from 3D cube",
      "<image3d in> <image2d out> <interval [us]> <NBcubes> <period> "
      "<offsetus> <sync stream name> <semtrig> <timing mode>",
      "creaimstream imcube imstream 1000 3 3 154 ircam1 3 0",
      "long COREMOD_MEMORY_image_streamupdateloop(const char *IDinname, const "
      "char *IDoutname, long usperiod, long NBcubes, long period, long "
      "offsetus, const char *IDsync_name, int semtrig, int timingmode)");

  RegisterCLIcommand(
      "creaimstreamstrig", __FILE__,
      ...
      "*IDinname, const char *IDoutname, long period, long offsetus, const "
      "char *IDsync_name, int semtrig, int timingmode)");
...
errno_t COREMOD_MEMORY_image_streamburst(const char *IDin_name,
  ...
  char *ptr0s = NULL;  // source start 3D array ptr
...
/** @brief takes a 3Dimage(s) (circular buffer(s)) and writes slices to a 2D
 * image with time interval specified in us
 *
 *
 * If NBcubes=1, then the circular buffer named IDinname is sent to IDoutname at
 * a frequency of 1/usperiod MHz If NBcubes>1, several circular buffers are
 * used, named ("%S_%03ld", IDinname, cubeindex). Semaphore semtrig of image
 * IDsync_name triggers switch between circular buffers, with a delay of
 * offsetus. The number of consecutive sem posts required to advance to the next
 * circular buffer is period
 *
 * @param IDinname      Name of DM circular buffer (appended by _000, _001
 * etc... if NBcubes>1)
 ...
 * @param period        If NBcubes>1: number of input triggers required to
 * advance to next input buffer
 * @param offsetus      If NBcubes>1: time offset [us] between input trigger and
 * input buffer switch
...
imageID COREMOD_MEMORY_image_streamupdateloop(
    const char *IDinname, const char *IDoutname, long usperiod, long NBcubes,
    long period, long offsetus, const char *IDsync_name, int semtrig,
  ...
  int sync_semwaitindex = 0;
  IDin = (long *)malloc(sizeof(long) * NBcubes);
  SyncSlice = 0;
  if (NBcubes == 1) {
    IDin[0] = image_ID(IDinname);

    // in single cube mode, optional sync stream drives updates to next slice
    // within cube
...
// takes a 3Dimage (circular buffer) and writes slices to a 2D image
// synchronized with an image semaphore
imageID COREMOD_MEMORY_image_streamupdateloop_semtrig(
    const char *IDinname, const char *IDoutname, long period, long offsetus,
    const char *IDsync_name, int semtrig,
    __attribute__((unused)) int timingmode) {
  imageID IDin;
  imageID IDout;
  imageID IDsync;
