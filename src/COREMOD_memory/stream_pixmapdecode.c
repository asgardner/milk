...
imageID COREMOD_MEMORY_PixMapDecode_U(
    ...
    const char *NBpix_fname, const char *IDmap_name, const char *IDout_name,
    const char *IDout_pixslice_fname, uint32_t reverse);
...
static errno_t COREMOD_MEMORY_PixMapDecode_U__cli() {
  if (0 + CLI_checkarg(1, CLIARG_IMG) + CLI_checkarg(2, CLIARG_LONG) +
          ...
          CLI_checkarg(7, CLIARG_STR_NOT_IMG) + CLI_checkarg(8, CLIARG_LONG) ==
      0) {
    COREMOD_MEMORY_PixMapDecode_U(
        data.cmdargtoken[1].val.string, data.cmdargtoken[2].val.numl,
        data.cmdargtoken[3].val.numl, data.cmdargtoken[4].val.string,
        data.cmdargtoken[5].val.string, data.cmdargtoken[6].val.string,
...
errno_t stream_pixmapdecode_addCLIcmd() {
  RegisterCLIcommand(
      "impixdecodeU", __FILE__, COREMOD_MEMORY_PixMapDecode_U__cli,
      "decode image stream",
      "<in stream> <xsize [long]> <ysize [long]> <nbpix per slice [ASCII "
      "file]> <decode map> <out stream> <out image slice index [FITS]> "
      "<reverse mode>",
      "impixdecodeU streamin 120 120 pixsclienb.txt decmap outim "
      "outsliceindex.fits 0",
      "COREMOD_MEMORY_PixMapDecode_U(const char *inputstream_name, uint32_t "
      "xsizeim, uint32_t ysizeim, const char* NBpix_fname, const char* "
      "IDmap_name, const char *IDout_name, const char *IDout_pixslice_fname, "
      "uint32_t reverse)");
...
imageID COREMOD_MEMORY_PixMapDecode_U(
  ...
  if (reverse == 0 && (xsizein != data.image[IDmap].md[0].size[0] ||
    ...
    printf("ERROR: xsize, ysize for %s (%d, %d) does not match %s (%d, %d)\n",
           inputstream_name, xsizein, ysizein, IDmap_name,
           data.image[IDmap].md[0].size[0], data.image[IDmap].md[0].size[0]);
    exit(0);
  }
  if (reverse == 1 && (xsizeim != data.image[IDmap].md[0].size[0] ||
                       ysizeim != data.image[IDmap].md[0].size[1])) {
    printf("ERROR: xsize, ysize for %s (%d, %d) does not match %s (%d, %d)\n",
           IDout_name, xsizein, ysizein, IDmap_name,
           data.image[IDmap].md[0].size[0], data.image[IDmap].md[0].size[0]);
    exit(0);
  }
  ...
  sizearray[0] = xsizeim;
  sizearray[1] = ysizeim;
  create_image_ID(IDout_name, 2, sizearray, data.image[IDin].md[0].datatype, 1,
                  25, 0, &IDout);
  ...
  for (slice = 0; slice < NBslice; slice++) {
    ...
    if (fscanfcnt == EOF) {
      ...
      fprintf(stderr,
              "Error: fscanf successfully matched and assigned %i input items, "
              "2 expected\n",
              fscanfcnt);
      return RETURN_FAILURE;
    }
  ...
  {
    create_image_ID("outpixsl", 2, sizearray, _DATATYPE_UINT16, 0, 0, 0,
                    &IDout_pixslice);
