...
errno_t COREMOD_MEMORY_testfunction_semaphore(const char *IDname, int semtrig,
                                              int testmode);

imageID COREMOD_MEMORY_image_NETWORKtransmit(const char *IDname,
                                             const char *IPaddr, int port,
                                             int mode, int RT_priority);

imageID COREMOD_MEMORY_image_NETWORKreceive(int port, int mode,
                                            int RT_priority);
...
errno_t stream__TCP_addCLIcmd() {
  ...
  RegisterCLIcommand(
      "imnetwreceive", __FILE__, COREMOD_MEMORY_image_NETWORKreceive__cli,
      "receive image(s) over network. mode=1 uses counter instead of semaphore",
      "<port [long]> <mode [int]> <RT priority>", "imnetwreceive 8887 0 80",
      "long COREMOD_MEMORY_image_NETWORKreceive(int port, int mode, int "
      "RT_priority)");
 ...
 * mode = 1, force counter to be used for synchronization, ignore semaphores if
 * they exist
 */
...
imageID COREMOD_MEMORY_image_NETWORKtransmit(const char *IDname,
  ...
  long long iter = 0;
  long framesize = 0;  // pixel data only
  ...
  TCP_BUFFER_METADATA *frame_md = NULL;
  ...
  if (loopOK == 1) {
    if (send(fds_client, (void *)data.image[ID].md, sizeof(IMAGE_METADATA),
      ...
      fflush(stdout);
      processinfo_error(
          processinfo, "send() sent a different number of bytes than expected");
      loopOK = 0;
    }
  }
  ...
  while (loopOK == 1) {
...
#else
      alarm(1);  // send SIGALRM to process in 1 sec - Will force sem_wait to
                 // proceed in 1 sec
    ...
    if (processinfo_compute_status(processinfo) == 1) {
      if (semr == 0) {
        ...
        ptr1 = ptr0 + framesize * slice;  // data.image[ID].md[0].cnt1; // frame
        ...
        if (rs != framesize1) {
          ...
          sprintf(errmsg,
                  "ERROR: send() sent a different number of bytes (%d) than "
                  "expected %ld  %ld  %ld",
                  rs, (long)framesize, (long)framesize1,
...
imageID COREMOD_MEMORY_image_NETWORKreceive(int port,
  ...
  // bind socket to port
  if (bind(fds_server, (struct sockaddr *)&sock_server, sizeof(sock_server)) ==
      -1) {
    char msgstring[200];
  ...
  if (data.processinfo == 1) {
    ...
    sprintf(processinfo->description, "%s %dx%dx%ld %s", imgmd[0].name,
            (int)xsize, (int)ysize, NBslices, typestring);
    processinfo_WriteMessage(processinfo, msgstring);
  }
