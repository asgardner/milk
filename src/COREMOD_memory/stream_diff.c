...
errno_t stream_diff_addCLIcmd() {
  RegisterCLIcommand(
      "streamdiff", __FILE__, COREMOD_MEMORY_streamDiff__cli,
      "compute difference between two image streams",
      "<in stream 0> <in stream 1> <out stream> <optional mask> <sem trigger "
      "index>",
      "streamdiff stream0 stream1 null outstream 3",
      "long COREMOD_MEMORY_streamDiff(const char *IDstream0_name, const char "
      "*IDstream1_name, const char *IDstreamout_name, long semtrig)");
...
imageID COREMOD_MEMORY_streamDiff(const char *IDstream0_name,
  ...
  if (IDout == -1) {
    create_image_ID(IDstreamout_name, 2, arraysize, _DATATYPE_FLOAT, 1, 0, 0,
                    &IDout);
